...
# 🔹 список посилань для моніторинга
RSS_OR_SEARCH_URLS = [
...
KEYWORDS = [
    ...
    "гном", "эльф", "средиземье", "бильбо", "фродо", "саурон", "орки", "79010", "79011", "79012"
]
...
def try_rss_parse(url):
    ...
    if feed and getattr(feed, "entries", None):
        for e in feed.entries:
            ...
            items.append({"id": uid, "title": title, "link": link, "price": price, "published": published})
    return items
...
def parse_html_search(url):
    try:
        ...
        for a in soup.select('a[href*="/d/"]'):
            ...
            if m:
                price = int(re.sub(r"[^\d]", "", m.group(1)))
            ads.append({"id": uid, "title": title, "link": link, "price": price, "published": ""})
...
def format_message(item):
    ...
    l = item.get("link")
    pub = item.get("published") or datetime.now().strftime("%Y-%m-%d %H:%M")
    msg = f"{t}\n{pr}\n{l}\n{pub}"
    return msg

def main():
    seen = load_seen()
    print("🔍 Моніторинг запущен...")
    while True:
        try:
            for url in RSS_OR_SEARCH_URLS:
                ...
                for it in items:
                    ...
                    if not uid:
                        continue
                    if not entry_passes_filters(it.get("title", ""), it.get("price")):
                        continue
                    if uid not in seen:
